...
// Feature-detect IE's crummy customer rect reporting for bidi text
var badBidiRects
...
function insertLineWidgetsFor(cm, line, lineView, dims, allowAbove) {
  ...
  for (var i = 0, ws = line.widgets; i < ws.length; ++i) {
    ...
    if (!widget.handleMouseEvents) { node.setAttribute("cm-ignore-event", "true") }
...
function eventInWidget(display, e) {
  for (var n = e_target(e); n != display.wrapper; n = n.parentNode) {
    if (!n || (n.nodeType == 1 && n.getAttribute("cm-ignore-event") == "true") ||
...
// Work around problem with bounding customer rects on ranges being
...
NativeScrollbars.prototype.enableZeroWidthBar = function (bar, delay) {
  ...
  function maybeDisable() {
    ...
    // it enabled, if it's hidden, we disable pointer event.
...
function markText(doc, from, to, options, type) {
  ...
  if (marker.replacedWith) {
    ...
    if (!options.handleMouseEvents) { marker.widgetNode.setAttribute("cm-ignore-event", "true") }
...
function onKeyPress(e) {
  ...
  // Some browsers fire keypress event for backspace
  if (ch == "\x08") { return }
...
function addEditorMethods(CodeMirror) {
  ...
  CodeMirror.prototype = {
    ...
    addWidget: function(pos, node, scroll, vert, horiz) {
      ...
      node.setAttribute("cm-ignore-event", "true")
...
ContentEditableInput.prototype.init = function (display) {
  ...
  on(div, "paste", function (e) {
    ...
    // IE doesn't fire input event, so we schedule a read for the pasted content in this way
