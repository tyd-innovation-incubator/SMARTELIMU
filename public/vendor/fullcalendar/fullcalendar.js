...
var ChronoComponent = Model.extend({
	...
	// Renders the event onto the view.
...
var Grid = FC.Grid = ChronoComponent.extend({
	...
	setElement: function(el) {
		...
		// attach event-element-related handlers. in Grid.event
	...
	// DOES NOT remove any content beforehand (doesn't clear event or call unrenderDates), unlike View
	removeElement: function() {
...
Grid.mixin({
	...
	// Compute business hour *event* for the grid's current date range.
	// Caller must ask if whole-day business hours are needed.
	// FOR RENDERING
...
Grid.mixin({
	...
	// Determines whether event should have their end times displayed, if not explicitly defined by 'displayEventTime'.
	// Only applies to non-all-day event.
	computeDisplayEventTime: function() {
		return true;
	},


	// Determines whether event should have their end times displayed, if not explicitly defined by 'displayEventEnd'
	...
	compareEventSegs: function(seg1, seg2) {
		...
		return r1.startMs - r2.startMs || // earlier event go first
			...
			f2.isAllDay - f1.isAllDay || // tie? put all-day event first (booleans cast to 0/1)
			compareByFieldSpecs(
...
Grid.mixin({
	...
	/* Event Helper
	------------------------------------------------------------------------------------------------------------------*/
	// TODO: should probably move this to Grid.event, like we did event dragging / resizing
...
Grid.mixin({
	...
	/* Fill System (highlight, background event, business hours)
	--------------------------------------------------------------------------------------------------------------------
...
/* Methods relate to limiting the number event for a given day on a DayGrid
----------------------------------------------------------------------------------------------------------------------*/
// NOTE: all the segs being passed around in here are foreground segs

DayGrid.mixin({

	segPopover: null, // the Popover that holds event that can't fit in a cell. null when not visible
...
/* Methods for rendering SEGMENTS, pieces of content that live on the view
 ( this file is no longer just for event )
----------------------------------------------------------------------------------------------------------------------*/
...
var View = FC.View = ChronoComponent.extend({
	...
	eventOrderSpecs: null, // criteria for ordering event when they have same date/time
	...
	// Signals that all event have been rendered
...
var Calendar = FC.Calendar = Class.extend(EmitterMixin, {
	...
	rerenderEvents: function() { // API method. destroys old event if previously rendered.
...
Calendar.mixin({
	...
	*/
	...
	// Returns a moment for the current date, as defined by the customer's computer or from the `now` option.
...
Calendar.mixin({
	...
	windowResize: function(ev) {
		if (
			!this.ignoreWindowResize &&
			ev.target === window && // so we don't process jqui "resize" event that have bubbled up
...
var BasicView = FC.BasicView = View.extend({
	...
	// Renders the given event onto the view and populates the segments array
	renderEventsPayload: function(eventsPayload) {
		this.dayGrid.renderEventsPayload(eventsPayload);

		// must compensate for event that overflow the row
...
var AgendaView = FC.AgendaView = View.extend({
	...
	// Renders event onto the view and populates the View's segment array
	renderEventsPayload: function(eventsPayload) {
		...
		// render the event in the subcomponents
		timedSegs = this.timeGrid.renderEventsPayload(timedEventsPayload);
		if (this.dayGrid) {
			daySegs = this.dayGrid.renderEventsPayload(dayEventsPayload);
		}

		// the all-day area is flexible and might have a lot of event, so shift the height
		// TODO: how will ChronoComponent handle this?
		this.updateHeight();
	},


	/* Dragging (for event and external elements)
	------------------------------------------------------------------------------------------------------------------*/
...
var ListViewGrid = Grid.extend({
	...
	renderFgSegs: function(segs) {
		segs = this.renderFgSegEls(segs); // might filter away hidden event
