        ...
        DB::transaction(function () use ($input, $model) {
            switch(class_basename($model)){
                case 'Company': {
                    ...
                    $model->reportIssues()->save($query);
                    return $query;
                }
                default : {
                    $query = $this->query()->create([
                        'user_id' => $model->user_id,
                        'company_id' => isset($model->company_id) ? $model->company_id : null,
                        'reported_by' => access()->id(),
                        'description' => $input['description'],
                        'report_issue_type_cv_id' => $input['type'],
                        'resource_id' => $model->id,
        ...
        DB::transaction(function () use ($input, $report) {
            ...
            switch ($input['status']) {
                case 1 : {
                    ...
                    //deactivate content
                    $resource = $model_name::find($report->resource_id);
//                          $resource->isactive = 0;
                    $resource->is_admin_disabled = 1;
                    $resource->save();
...
//                     $report->user->notify(new ReportDeclineNotification());
    ...
    public function getCountPendingsReportIssues()
    {
        return $this->query()->where('status',0)->count();
    }
}
